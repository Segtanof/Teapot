...
dsampled_occupation = sampled_occupation[:8]
...
async def run_batch():
    responses = await llm.abatch(args)
    return responses

def get_rating(title, model, system_prompt=None, batch_size=60):
        ...
        while True:  # Infinite loop until all ratings are valid
            ...
            try:
                with torch.cuda.device(0):
                    ...
                    responses = await llm.abatch(prompts, config={"num_threads": 4})
