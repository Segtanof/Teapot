...
for model in model_list:
    ...
    for name, prompt in prompts.items():
        ...
        for i in range(5):
            ...
            for title, gen_task in results:
                result_df.loc[result_df["title"] == title, "gen_task"] = gen_task
            ...
            try:
                ...
                with open(f"{folder_name}/{model_name}_{name}_{i}_sim.json", "w") as f:
                    f.write(result_df.to_json(index=True))
            except Exception as e:
                print(f"Error in {model_name}-{name}-{i}: {e}")
