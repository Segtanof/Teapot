...
title_rates = df.groupby(['prompt', 'industry', 'title'])['correct_match'].mean().reset_index(name='mean_correct_match')
...
df = df[~df['industry'].isin(["17", "37", "45", "49"])]
...
# Set industry as factor with '29' as first level in R
r('''
if ("29" %in% levels(r_df$industry)) {
    r_df$industry <- factor(r_df$industry, levels = c("29", setdiff(levels(r_df$industry), "29")))
} else {
    message("Warning: '29' not found in industry levels. Using default order.")
}
''')
