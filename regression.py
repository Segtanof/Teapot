...
coefficients_file = "model_coefficients.txt"
random_effects_file = "random_effects.txt"
...
if r_model_exists:
    ...
    r(f'write.table(coef(summary(r_model)), file="{coefficients_file}", sep="\\t", quote=FALSE, row.names=TRUE)')
    print(f"Model coefficients saved to: {coefficients_file}")

    # --- Save the random effects variance to a text file ---
    r(f'sink("{random_effects_file}")')
