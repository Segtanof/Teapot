...
# Ensure zone is a factor with '3' as reference
if ("3" %in% levels(r_df$zone)) {
    r_df$zone <- factor(r_df$zone, levels = c("3", setdiff(levels(r_df$zone), "3")))
} else {
...
# Fit initial GLMM
initial_model <- glmer(
    correct_match ~ prompt * zone + (1 | title),
    data = r_df,
...
initial_outlier_summary <- table(initial_outliers$zone, initial_outliers$prompt)
...
initial_top_residuals <- head(r_df[order(-abs(r_df$resid)), c("title", "zone", "prompt", "resid")], 10)
...
cat("Initial Outlier Summary by Zone and Prompt:\n")
